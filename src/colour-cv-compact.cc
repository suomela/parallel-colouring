...
static void reduce12_6(unsigned n, const unsigned *p, col_t *c, const col_t *cold) {
    ...
    unsigned n2 {n/2};
    FOREACH(i, n2, c[i] = pack2(
        reduce(unpack2(c[i], 0), get2(cold, p[2*i + 0])),
        reduce(unpack2(c[i], 1), get2(cold, p[2*i + 1]))
    ));
}
...
static void reduce6_4(unsigned n, const unsigned *p, col_t *c, const col_t *cold) {
    ...
    FOREACH(i, n4, c[i] = pack4(
        reduce(unpack2(c[2*i + 0], 0), get2(cold, p[4*i + 0])),
        reduce(unpack2(c[2*i + 0], 1), get2(cold, p[4*i + 1])),
        reduce(unpack2(c[2*i + 1], 2), get2(cold, p[4*i + 2])),
        reduce(unpack2(c[2*i + 1], 3), get2(cold, p[4*i + 3]))
    ));
}
